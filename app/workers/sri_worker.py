...
#
# Worker event-driven para procesamiento de facturas con el SRI.
# Usa Redis como señalizador — la DB es siempre la fuente de verdad.
#
# Flujo:
#   1. Al arrancar: recovery desde DB (facturas FIRMADO/RECIBIDA sin procesar)
#   2. En operación: BRPOPLPUSH — duerme hasta que llegue un ID
#   3. Si Redis se cae/reinicia: el recovery al arrancar reconstruye las colas
...
# ── Claves Redis ───────────────────────────────────────────────────────────────
QUEUE_EMISION       = "kipu:queue:emision"        # facturas FIRMADO esperando envío
QUEUE_AUTORIZACION  = "kipu:queue:autorizacion"   # facturas RECIBIDA esperando autorización
QUEUE_PROC_EMISION  = "kipu:queue:proc:emision"   # en proceso (evita pérdida si worker cae)
QUEUE_PROC_AUTH     = "kipu:queue:proc:auth"       # en proceso autorización
...
MAX_CONCURRENT      = 3       # máximo de requests simultáneos al SRI
MAX_RETRIES         = 5
BRPOP_TIMEOUT       = 5       # segundos que espera BRPOP antes de hacer loop

# ── Semáforo global para limitar concurrencia al SRI ──────────────────────────
...
async def recovery_al_arrancar():
    """
    Al iniciar el worker, recupera desde la DB todas las facturas
    que quedaron pendientes (por crash, reinicio, cambio de servidor, etc.)
    y las mete en Redis para que el worker las procese.
    """
    print("[Recovery] 🔍 Buscando facturas pendientes en DB...")
    redis = await get_redis()

    async with AsyncSessionLocal() as db:
        # Facturas que nunca llegaron al SRI o quedaron a medias
        ...
        # Facturas recibidas por el SRI pero que no tienen autorización aún
    ...
    # Limpiar colas de "en proceso" huérfanas del arranque anterior
...
def es_unlimited(unlimited) -> bool:
    """Verifica si la factura viene de un servicio interno ilimitado."""
    return bool(unlimited)


# =============================================================================
# PROCESAMIENTO INDIVIDUAL — EMISIÓN
# =============================================================================

async def procesar_emision(factura_id: str):
    """Envía una factura FIRMADO al SRI (recepción). Respeta semáforo."""
    async with _sri_semaphore:
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not factura:
                    # Ya fue procesada por otro worker o fast-track — ok
                ...
                res = await httpx_with_retry(
                    urls["recepcion"], soap_body, {"Content-Type": "text/xml"}
                )
                ...
                if resp_recepcion and resp_recepcion.get("estado") == "RECIBIDA":
                    ...
                    # Pasar a cola de autorización
                    redis = await get_redis()
                    await redis.lpush(QUEUE_AUTORIZACION, str(factura.id))

                else:
                    # DEVUELTA — el XML tiene errores, devolver crédito si no es ilimitado
                ...
                await db.rollback()
                # Incrementar retry y devolver a la cola si no superó el límite
...
# =============================================================================
# PROCESAMIENTO INDIVIDUAL — AUTORIZACIÓN
# =============================================================================

async def procesar_autorizacion(factura_id: str):
    """Consulta autorización al SRI para una factura en estado RECIBIDA."""
    async with _sri_semaphore:
        async with AsyncSessionLocal() as db:
            try:
                result = await db.execute(text("""
                ...
                res = await httpx_with_retry(
                    urls["autorizacion"], soap_body, {"Content-Type": "text/xml"}
                )
                ...
                if not resp_auth or int(resp_auth.get("numeroComprobantes", 0)) == 0:
                    # SRI aún no tiene respuesta — reintentar después
                ...
                if autorizacion.get("estado") == "AUTORIZADO":
                    ...
                    # Generar PDF para email — no se guarda en R2
                    pdf_para_email = None
                    try:
                    ...
                    # Disparar webhooks para sistemas externos (Komand, Wappti, etc.)
                    await disparar_webhooks(factura.id, factura.emisor_db_id, "factura.autorizada", fac_dict)

                    # Email con XML + PDF
                    if factura.email_comprador:
                        attachments = [{
                            "filename":  f"{factura.clave_acceso}.xml",
                            "content":   xml_autorizado.encode("utf-8"),
                            "maintype":  "text",
                            "subtype":   "xml"
                        }]
                        if pdf_para_email:
                    ...
                    # Devolver crédito solo si no es ilimitado
                    if not es_unlimited(factura.unlimited):
                        await db.execute(
                            text("UPDATE user_credits SET balance_emision = balance_emision + 1 WHERE emisor_id = :eid"),
                            {"eid": factura.emisor_db_id}
                ...
                # Reintentar autorización después de una pausa
...
# =============================================================================
# WEBHOOKS — notificar a sistemas externos
# =============================================================================

async def disparar_webhooks(factura_id, emisor_id: int, evento: str, payload: dict):
    """Notifica a todos los webhooks configurados para este emisor y evento."""
    async with AsyncSessionLocal() as db:
        try:
            result = await db.execute(text("""
...
# =============================================================================
# LOOPS PRINCIPALES — event-driven con BRPOPLPUSH
# =============================================================================

async def loop_emision():
    """
    Loop principal de emisión.
    Usa BRPOPLPUSH: mueve el ID a la lista 'proc' atómicamente.
    Si el worker cae, el recovery al reiniciar recupera los huérfanos.
    """
    ...
    while True:
        try:
            # Bloquea hasta 5 segundos esperando un ID — si no llega, hace loop
            resultado = await redis.brpoplpush(QUEUE_EMISION, QUEUE_PROC_EMISION, timeout=BRPOP_TIMEOUT)
            if resultado is None:
                continue  # timeout, sin facturas — vuelve a esperar

            factura_id = resultado
            # Procesar en background para no bloquear el loop
            asyncio.create_task(
                _procesar_y_limpiar_emision(factura_id, redis)
            )
...
async def _procesar_y_limpiar_emision(factura_id: str, redis):
    """Procesa la emisión y limpia la lista de 'en proceso' al terminar."""
    try:
...
async def loop_autorizacion():
    """Loop principal de autorización — igual que emisión."""
    print("[Worker] 🚀 Loop de autorización iniciado.")
    redis = await get_redis()

    while True:
        try:
            resultado = await redis.brpoplpush(QUEUE_AUTORIZACION, QUEUE_PROC_AUTH, timeout=BRPOP_TIMEOUT)
            if resultado is None:
                continue

            factura_id = resultado
            asyncio.create_task(
                _procesar_y_limpiar_auth(factura_id, redis)
            )
...
# =============================================================================
# PUNTO DE ENTRADA — llamado desde main.py lifespan
# =============================================================================
...
async def iniciar_workers():
    ...
    1. Recovery desde DB
    2. Loops event-driven en paralelo
    """
