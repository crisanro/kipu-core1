...
#
# Endpoints unificados para emisión y consulta de comprobantes electrónicos.
# Reemplaza invoices.py y notas_credito.py
#
...
class EmitirDocumentoRequest(BaseModel):
    ...
    # NCR / NDB — doc origen en Kipu (emitido)
    doc_origen_id:         Optional[str]        = None
    motivo:                Optional[str]        = None
    motivos:               Optional[list[dict]] = None

    # Doc origen manual (no está en Kipu)
    doc_origen_numero:     Optional[str]        = None  # "001-001-000000123"
    doc_origen_fecha:      Optional[str]        = None  # "21/10/2012"
    doc_origen_cod_doc:    Optional[str]        = None  # "01" | "03"
    cliente_origen:        Optional[dict]       = None  # {tipo_id, identificacion, razon_social}

    # RET — desde doc recibido (FAC/NDB que nos hicieron)
    doc_origen_recibido_id: Optional[str]        = None
    # RET — desde doc emitido (LIQ que nosotros emitimos)
    doc_origen_emitido_id:  Optional[str]        = None  # RET desde LIQ
...
async def historial_documentos(
    auth_data:    dict         = Depends(verify_firebase_token),
    db:           AsyncSession = Depends(get_db),
    tipo_doc:     Optional[str] = Query(None, description="FAC|LIQ|NCR|NDB|RET"),
...
# =============================================================================
# RESUMEN DECLARACIÓN — GET /resumen
# =============================================================================
...
async def resumen_documentos(
    ...
    # ── Resumen FAC / LIQ / NCR / NDB — por tarifa de IVA ───────────────────
    ...
    # ── Resumen RET — por tipo de impuesto (Renta vs IVA) ───────────────────
    ...
    # Agrupar retenciones por tipo (Renta / IVA / ISD)
    ret_agrupado: dict = {}
        ...
        SELECT
            ...
            (
                SELECT json_agg(json_build_object(
                    'id',            dd.id,
                    'tipo_doc',      dd.tipo_doc,
                    'numero_doc',    dd.numero_doc,
                    'estado_sri',    dd.estado_sri,
                    'importe_total', dd.importe_total,
                    'created_at',    dd.created_at
                ) ORDER BY dd.created_at ASC)
                FROM documentos_emitidos dd
                WHERE dd.doc_origen_emitido_id = d.id
            ...
            (
                SELECT json_build_object(
                    'id',            dp.id,
                    'tipo_doc',      dp.tipo_doc,
                    'numero_doc',    dp.numero_doc,
                    'estado_sri',    dp.estado_sri,
                    'importe_total', dp.importe_total
                )
                FROM documentos_emitidos dp
            ...
            (
                SELECT json_build_object(
                    'id',                     dr.id,
                    'numero_doc',             dr.numero_doc,
                    'ruc_proveedor',          dr.ruc_proveedor,
                    'razon_social_proveedor', dr.razon_social_proveedor,
                    'importe_total',          dr.importe_total,
                    'fecha_emision',          dr.fecha_emision
                )
                FROM documentos_recibidos dr
                WHERE dr.id = d.doc_origen_recibido_id
...
async def reintentar_documento(
    ...
    if doc.estado_sri not in ("DEVUELTA", "RECHAZADO", "FIRMADO"):
        raise HTTPException(
            status_code=400,
            detail=f"No se puede reintentar un documento en estado {doc.estado_sri}."
        )

    await db.execute(text("""
        UPDATE documentos_emitidos
        SET estado_sri   = 'FIRMADO',
            mensajes_sri = NULL,
            updated_at   = NOW()
        WHERE id = :did AND emisor_id = :eid
    """), {"did": doc_id, "eid": emisor_id})
    ...
    return {
        "ok":      True,
        "mensaje": "Documento reencolado para reintento.",
        "estado":  "FIRMADO",
    }


# =============================================================================
# ESTADO DE COBRO — PATCH /{doc_id}/cobro
# =============================================================================
...
async def actualizar_cobro(
    ...
    res = await db.execute(text("""
        SELECT id, tipo_doc, estado_sri FROM documentos_emitidos
        WHERE id = :did AND emisor_id = :eid
    """), {"did": doc_id, "eid": emisor_id})
    doc = res.fetchone()
    ...
    if doc.tipo_doc not in ("FAC", "LIQ"):
        raise HTTPException(
            status_code=400,
            detail="El estado de cobro solo aplica a facturas y liquidaciones."
        )
    ...
    return {
        "ok":      True,
        "mensaje": f"Estado de cobro actualizado a {data.estado_cobro}.",
    }
